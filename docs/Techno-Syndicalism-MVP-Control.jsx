                ...
                return(
                    <div>
                        <button onClick={this.handleClickBlank}>Back</button>
                            ...
                            <input type="text" placeholder="Union Name"></input>
        ...
        switch(this.state.union){
            case "all":
                 return (
                     <div> 
                         <p>My Unions</p>
                         <button onClick={this.handleClickDeliveroo}><p>Deliveroo Newcastle</p></button>
                         <button onClick={this.handleClickBurger}><p>Burger King Northumberland Street</p></button>
                     </div>
                        );
                break;
            case "Deliveroo":
                return (
                    <Union name="Deliveroo Newcastle"></Union>
                );
                break;
            case "Burger":
                return (
                    <Union name="Burger King Northumberland Street"></Union>
                );
        }
...
class Union extends React.Component {
    ...
    render(){
        return (
            <div>
                <p>{this.state.name}</p>
                <button><p>Members</p></button>
                <button><p>Chat</p></button>
                <button><p>Funds</p></button>                
                <button><p>Union Info</p></button>
            </div>
    ...
    render(){
        if(this.state.results){
            return (
                <div>
                    <p>Results:</p>
                    <button><p>Greggs Byker</p></button>
                    <button><p>Topman Eldon Square</p></button>
                    <button><p>Deliveroo Newcastle</p></button>
                    <button><p>Burger King Northumberland Street</p></button>   
                </div>
                   );
        }
        else {
            return (
            <div>
                <input type="text" placeholder="Workplace"></input>
...
class App extends React.Component {
    ...
    render(){
       ...
       if(this.state.loggedin){
           return(
               <div><Welcome name={this.state.name}/>
               <p onClick={this.handleClickLogout} class="w3-text-red w3-card">Log Out</p></div>
           );
       }
        else{
            return (
